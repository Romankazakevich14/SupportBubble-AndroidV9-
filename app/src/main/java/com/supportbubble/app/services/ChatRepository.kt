...
class ChatRepository(
    ...
    /**
     * Reactive count of unread admin messages in a single per-app thread.
     * Used to show the red "unread" dot on the overlay chat Refresh button.
     */
    fun unreadCountForPackage(packageName: String): Flow<Int> =
        messageDao.observeUnreadAdminCountForPackage(deviceId, packageName)
    ...
    /**
     * Manual / pull-to-refresh sync. Pulls the latest messages from the server
     * into the shared Room DB; the message Flow then re-emits automatically and
     * the chat updates instantly.
     *
     * Channel selection (per the product requirement):
     * - If the live socket is connected, also flush any queued outgoing messages
     *   so a manual refresh doubles as a "kick the queue" action.
     * - If the socket is NOT connected (the case this whole feature exists for —
     *   admin messages not arriving while MainActivity is closed), re-attempt the
     *   socket connection AND fall back to the REST history endpoint to sync.
     *
     * Either way the REST history fetch is the reliable pull mechanism, so it
     * always runs. Works even with MainActivity fully closed because the
     * repository owns its own HTTP client and the Room DB is process-wide.
     *
     * @return true if the server history fetch succeeded.
     */
    suspend fun refreshMessages(): Boolean {
        if (socketManager.isConnected) {
            Log.d(LOG_CHAT_SESSION, "refresh: socket connected — flushing queue + REST sync")
            flushPendingMessages()
        } else {
            Log.d(LOG_CHAT_SESSION, "refresh: socket down — reconnecting + REST sync")
    ...
    fun markAsRead(packageName: String) {
        socketManager.emitRead(packageName)
        scope.launch(Dispatchers.IO) {
    ...
    /**
     * Awaitable variant of [markAsRead]: suspends until the local Room write
     * completes. Used by the panel-open flow so the mark-read can be ordered
     * strictly AFTER the history fetch upsert (prevents a late fetch from
     * re-showing the unread dot).
     */
    suspend fun markAsReadAwait(packageName: String) {
        socketManager.emitRead(packageName)
        messageDao.markAdminMessagesReadForPackage(deviceId, packageName)
    }
     ...
     */
    suspend fun fetchHistory(): Boolean {
        val request = Request.Builder()
        ...
        return try {
            http.newCall(request).execute().use { response ->
                ...
                if (json.get("success")?.asBoolean != true) return@use false

                val data = json.getAsJsonObject("data") ?: return@use false
                val messagesArr = data.getAsJsonArray("messages") ?: return@use false
